...
$title = "Mortgage Loan Limits for $county";
$description = YEAR . " $county, Loan Limits — 1-Unit Conforming $" . number_format($data['oneUnitConforming'][YEAR]) . ", 1-Unit FHA " . number_format($data['oneUnitFha'][YEAR]) . ". Charts included. Updated regularly.";
...
<div class="post lg:flex pt-5 md:pt-10 bg-brand-purple-lightest overflow-clip">
  ...
  <div class="z-0">
    ...
    <div class="max-w-2xl mx-auto relative px-6 md:px-0">
      ...
      <!-- Title -->
      <h1 class="mb-6">Mortgage Loan Limits: <?php echo $county; ?></h1>
      <!-- Notice Block -->
    ...
    <div class="post py-14 bg-white">
      ...
      <div class="px-6 md:px-0 max-w-2xl mx-auto">
        ...
        <script>
          window.addEventListener('load', function () {
            renderMap("<?php echo $county; ?>", "county", "loan-limits-map");
          });
        </script>
        <h2>What Is A Mortgage Loan Limit?</h2>
        <p><a href="<?php echo home_url(); ?>/mortgage-loan-limits">Mortgage loan limits</a> are the upper boundaries for how much a home buyer can borrow on a mortgage. Loan limits vary by home type and county, with some counties receiving the <a href="<?php echo home_url(); ?>/learn/high-cost-area">high-cost areas</a> designation.</p>
        ...
        <div class="grid gap-4 md:gap-8 grid-cols-2 md:grid-cols-3 mb-14">
          <?php
            ...
            get_template_part(
              ...
              null,
              [
                'url' => home_url() . '/mortgage-loan-limits#mortgage-stats',
                'title' => 'Mortgage Statistics',
                'link_text' => 'National Data',
              ],
            );
